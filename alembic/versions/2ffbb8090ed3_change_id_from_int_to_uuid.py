...
def upgrade() -> None:
    ...
    op.create_table(
        'raw_data',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, comment='主键ID'),
        sa.Column('name', sa.String(100), nullable=False, comment='原始数据名称'),
        sa.Column('description', sa.Text(), nullable=True, comment='原始数据描述'),
        sa.Column('raw_type', sa.String(20), nullable=False, comment='原始数据类型: database_table/file'),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False, comment='所属用户ID'),
    ...
    op.create_table(
        ...
        sa.Column('raw_data_id', postgresql.UUID(as_uuid=True), nullable=False, comment='原始数据ID'),
