...
- 目标：先登记“原始数据”（库表/文件），再组合成“数据源”并在分析会话中选择使用。
- 故事：用户登录 → 在“原始数据”中新建并预览 raw 对象 → 在“数据源管理”创建数据源、挑选 raw 并做字段对齐 → 在分析会话侧边栏选择数据源开始提问。

## 端到端流程（原始数据→数据源→分析）
1. 配置原始数据：新建 raw（库表或文件），填写连接/上传信息，点击预览抽样，命名并可修正类型。
...
## 差距与待办（对齐 PRD）
- 缺少“原始数据 raw”层：需要新增 raw 创建/预览/管理，缓存抽样 schema。
...
- 前端：需新增“原始数据”页与“数据源编辑器（字段对齐+预览）”，会话侧数据源选择需展示映射后的逻辑字段。
...
5. 用户可以：
   - 给这个原始数据起一个名字（逻辑名），比如 `pg_orders_raw`、`s3_events_raw`；
...
### 原始数据配置页
- 入口：导航“数据源管理”下的“原始数据”页；按钮“新建原始数据”。
...
## 这样做的优点
   ...
   - 步骤简单清晰：
     1. 配原始数据（怎么连、在哪里、长什么样）
     2. 用原始数据组合“数据源”（给 AI 用的入口）
